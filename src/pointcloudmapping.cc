...
/**
 * @brief: 函数先遍历每一个关键帧的像素点，看每个像素点周围20*20的小方格内是否有人存在，如果存在的话，那么改像素点将不参与建图，DS-SLAM正是用这种方法来达到在建图中消除人的作用的。flag_exist就是改像素点周围环境是否含有人的标记 如果改像素点没有人的话，那么开启建图。不过DS-SLAM通过查看在语义分割图上该像素的灰度值来判断该像素点是否有语义信息，如果有语义信息就将语义分割图片的颜色赋给点云，颜色信息从2D映射到3D；否则点云的信息就是正常RGBD的颜色，代码如下：
 * @param {KeyFrame*} kf
 * @param {Mat&} semantic_color
 * @param {Mat&} semantic
 * @param {Mat&} color
 * @param {Mat&} depth
 * @return {*}
